...
class InteractiveSocialNavRandomTask(SocialNavRandomTask):
    """
    ...
    def reset_interactive_objects(self, env):
        ...
        if self.offline_eval:
            ...
            assert len(self.all_interactive_objects) > 0
            self.interactive_objects_idx = np.random.choice(
                np.arange(len(self.all_interactive_objects)),
                min(num_interactive_objects, len(self.all_interactive_objects)), 
                replace=False)
    ...
    def reset_scene(self, env):
        ...
        :param env: environment instance
        """
        super(InteractiveSocialNavRandomTask, self).reset_scene(env)
    ...
    def reset_agent(self, env):
        ...
        :param env: environment instance
        """
        super(InteractiveSocialNavRandomTask, self).reset_agent(env)
        if self.offline_eval:
    ...
    def step(self, env):
        super(InteractiveSocialNavRandomTask, self).step(env)
    ...
    def get_termination(self, env, collision_links=[], action=None, info={}):
        ...
        """
        done, info = super(InteractiveSocialNavRandomTask, self).get_termination(
            env, collision_links, action, info)

        for metric in ['effort_efficiency', 'ins']:
            for dist_grid in ['0-4', '4-8', '8-12', '12-16', '16-20']:
                info[f'{metric}_{dist_grid}m'] = 0.0
        ...
        grid = int(self.geodesic_dist / 4)
        info[f'effort_efficiency_{grid * 4}-{grid * 4 + 4}m'] = info['effort_efficiency']
        info[f'ins_{grid * 4}-{grid * 4 + 4}m'] = info['ins']
